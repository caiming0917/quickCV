...
public class ArrayHandler implements TypeHandler<List<?>> {

    @Override
    public void setParameter(PreparedStatement statement, int i, List<?> parameter, JdbcType jdbcType) throws SQLException {
        if (parameter == null) {
            try {
                statement.setNull(i, JdbcType.ARRAY.TYPE_CODE);
            ...
            try {
                statement.setArray(i, statement.getConnection().createArrayOf(jdbcType.name(), parameter.toArray()));
            } catch (Exception e) {
